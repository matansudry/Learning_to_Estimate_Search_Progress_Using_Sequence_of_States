...
def astar_search(
    ...
    while open_list:
        ...
        node_dict[pop_node.state] = ((cnt,len(task.get_successor_states(pop_state))))
        if cnt == 1: # if i am root
            father_n = None
            temp_father_BF = None
        else: # if i am not root
            father_n = node_dict[pop_node.parent.state][0]
            temp_father_BF = node_dict[pop_node.parent.state][1]
        node_values[cnt] = ((father_n, f, h, f-h, temp_father_BF)) # (parent#, f, h, g)
        ...
        if (pop_node.parent == None): # i dont have father and i am root
            ...
            father_BF = node_values[cnt][4]

            if node_values[cnt][0] == 1: # my father is rrot
                grandfather_n = 0
                grandfather_f = 0
                grandfather_h = 0
                grandfather_g = 0
                ...
                grandfather_BF = node_values[father_n][4]
        if (h0 == 0):
            h0 = h
        ...
        resutls.append((cnt, f, BF, h, f-h, father_n, father_BF, father_f, father_h, father_g, grandfather_n, grandfather_BF,grandfather_f, grandfather_h, grandfather_g, h0, H_min, cnt - last_H_min_update, f_max))
        ...
        if state_cost[pop_state] == pop_node.g:
            ...
            if task.goal_reached(pop_state):
                with open(output_name+'.csv', 'w', newline='') as myfile:
                    ...
                    temp = ["node#", "node_f",  "BF", "node_h", "node_g", "father_node#", "father_BF", "father_node_f", "father_node_h", "father_node_g",
                            "grandfather_node#", "grandfather_BF", "grandfather_node_f", "grandfather_node_h", "grandfather_node_g", "H0", "H_min", "last_H_min_update", "f_max","node_max"]
                    wr.writerow(temp)
