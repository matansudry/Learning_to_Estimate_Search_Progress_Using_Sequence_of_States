    ...
    for planner in ["gbfs_hff", "astar_hff", "astar_lmcut", "gbfs_lmcut"]:
        ...
        for path_tuple in paths:
            ...
            if os.path.isfile(os.getcwd()+path_tuple[1]) == False:
                continue
            column = ["level_1#", "level_2#", "H_min", "node_max"]
            ...
            for i in tqdm.tqdm(range(len(df))):
                exp_delay = df[i][0] - df[i][1]
                exp_delays.append(exp_delay)
                avg_exp_delays = (avg_exp_delays * i + exp_delay) / (i+1)
                vasp.append((df[i][0]) / (df[i][0]+(df[i][2]*avg_exp_delays))*100)
                y.append(((df[i][0]/df[i][3]*100) - vasp[i])*((df[i][0]/df[i][3]*100) - vasp[i]))
            x_df = pd.DataFrame(vasp)
            words = path.split("/")
            x_df.to_csv("vasp/"+words[-2]+"-"+words[-1])
            y_df = pd.DataFrame(y)
            y_df.to_csv("vasp/"+planner+"_y_"+words[-2]+"-"+words[-1])
